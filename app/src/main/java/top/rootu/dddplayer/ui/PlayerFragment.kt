...
class PlayerFragment : Fragment() {
    ...
    private fun observeViewModel() {
        ...
        viewModel.duration.observe(viewLifecycleOwner) { duration ->
            ...
            android.util.Log.d("DDDPlayer/UI", "seekbar.setMax max=${ui.seekBar.max} requested=$safeDuration visibility=${ui.seekBar.visibility} width=${ui.seekBar.width}")
        ...
        viewModel.currentPosition.observe(viewLifecycleOwner) { position ->
            if (!viewModel.isUserInteracting) {
                ...
                android.util.Log.d("DDDPlayer/UI", "seekbar.setProgress progress=${ui.seekBar.progress} requested=$safePosition max=${ui.seekBar.max} visibility=${ui.seekBar.visibility} width=${ui.seekBar.width}")
        ...
        viewModel.videoTitle.observe(viewLifecycleOwner) { title ->
            ...
            android.util.Log.i("DDDPlayer/UI", "videoTitle update title=$title poster=$posterUri index=$index size=$size")
        }
    ...
    private fun updateTimeLabelsUI(currentPos: Long) {
        ...
        val speed = viewModel.playbackSpeed.value?.value ?: 1.0f
        val isLive = viewModel.isLive.value ?: false
        val liveOffset = viewModel.player?.currentLiveOffset ?: 0L
