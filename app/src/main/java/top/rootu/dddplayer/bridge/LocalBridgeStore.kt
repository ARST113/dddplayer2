...
class LocalBridgeStore(private val maxEvents: Int = 200) {
    ...
    fun append(envelope: BridgeEnvelope) = synchronized(lock) {
        ...
        stateView[k] = when (val p = envelope.payload) {
            is BridgeEvent.SessionStarted -> LocalBridgeState(
            ...
            is BridgeEvent.PlaybackEnded -> (prev ?: LocalBridgeState(envelope.schema,envelope.sessionId,envelope.client,envelope.ts,p.uri,p.position,p.duration,null,null,p.title,p.windowIndex,p.playlistSize,null,null,envelope.type,"ended",false,null,null,null,null)).copy(updatedAt = envelope.ts, uri = p.uri, position = p.position, duration = p.duration, title = p.title, windowIndex = p.windowIndex, playlistSize = p.playlistSize, lastEventType = envelope.type, lastReason = "ended", finished = alreadyFinished, endBy = prev?.endBy)
            ...
            is BridgeEvent.Error -> (prev ?: LocalBridgeState(envelope.schema,envelope.sessionId,envelope.client,envelope.ts,p.uri,p.position,p.duration,p.bufferedPosition,p.bufferedPercentage,p.title,p.windowIndex,p.playlistSize,null,null,envelope.type,"error",false,null,null,null,null)).copy(updatedAt = envelope.ts, uri = p.uri, position = p.position, duration = p.duration, bufferedPosition = p.bufferedPosition, bufferedPercentage = p.bufferedPercentage, title = p.title, windowIndex = p.windowIndex, playlistSize = p.playlistSize, lastEventType = envelope.type, lastReason = "error", error = p, finished = if (p.fatal) true else alreadyFinished, endBy = if (p.fatal) "error" else prev?.endBy)
