...
class PlayerViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private var lastNonZeroDurationMs: Long = 0L
        ...
        override fun run() {
            ...
            if (isActive) {
                ...
                _bufferedPosition.value = playerManager.getBufferedPositionMs().coerceAtLeast(0L)
                                val newIndex = playerManager.getCurrentWindowIndex()
                if (_currentWindowIndex.value != newIndex) _currentWindowIndex.value = newIndex
                val newPlaylistSize = playerManager.getPlaylistSize()
                if (_playlistSize.value != newPlaylistSize) _playlistSize.value = newPlaylistSize
                val newHasPrevious = playerManager.hasPrevious()
                if (_hasPrevious.value != newHasPrevious) _hasPrevious.value = newHasPrevious
                val newHasNext = playerManager.hasNext()
                if (_hasNext.value != newHasNext) _hasNext.value = newHasNext
                val dNow = playerManager.getDurationMs().takeIf { it > 0 }
                if (dNow != null && _duration.value != dNow) {
                    _duration.value = dNow
                    android.util.Log.d("DDDPlayer/Backend", "duration from backend=$dNow")
                }
    ...
    init {
        ...
        playerManager.onBackendPositionChanged = { positionMs, durationMs ->
            if (!isUserInteracting) {
                _currentPosition.postValue(positionMs.coerceAtLeast(0L))
            }
    ...
    private fun buildVlcSubtitleTrackOption(track: BackendSubtitleTrack, ordinal: Int): TrackOption {
        if (track.id < 0) {
            return TrackOption(
                format = null,
                nameFromMeta = getString(R.string.track_off),
                index = -1,
                group = null,
                trackIndex = track.id,
                isOff = true
        ...
        val media3Options = subtitleOptions.filterNot { it.isOff }
        val matched = media3Options.firstOrNull { it.index == track.id }
            ?: media3Options.getOrNull(ordinal.coerceAtLeast(0))
