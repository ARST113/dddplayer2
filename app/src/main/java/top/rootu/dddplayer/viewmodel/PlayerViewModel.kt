...
class PlayerViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun refreshCurrentPlaylistUiState(reason: String) {
        ...
        _currentPosition.value = playerManager.getPositionMs().coerceAtLeast(0L)
        _duration.value = normalizeDuration(playerManager.getDurationMs())
        _hasPrevious.value = playerManager.hasPrevious()
        _hasNext.value = playerManager.hasNext()
        Log.i(TAG, "refreshCurrentPlaylistUiState reason=$reason idx=${_currentWindowIndex.value} title=${_videoTitle.value}")
    }
    ...
    private fun refreshCurrentAudioTrackUiState() {
        ...
        val selectedId = playerManager.getVlcSelectedAudioTrack()
        val selectedPair = tracks.firstOrNull { it.first == selectedId }
        _currentAudioTrack.value = selectedPair?.let {
            TrackOption(format = null, nameFromMeta = it.second, index = it.first, group = null, trackIndex = it.first)
        }
        Log.i("DDDPlayer/VLC", "refreshCurrentAudioTrackUiState selectedId=$selectedId displayed=${selectedPair?.second}")
    }
    ...
    fun selectTrackByIndex(trackType: Int, index: Int, persist: Boolean = true, reason: String = "user_selected", matchScore: Int? = null) {
        ...
        if (playerManager.getActiveBackendId() == "VLC") {
            if (trackType == C.TRACK_TYPE_AUDIO) {
                ...
                val selectedBefore = playerManager.getVlcSelectedAudioTrack()
                android.util.Log.i("DDDPlayer/VLC", "audio_select backend=${playerManager.getActiveBackendId()} tracks=$list selectedBefore=$selectedBefore targetIndex=$index")
                val pair = list.getOrNull(index) ?: list.firstOrNull { it.first == index } ?: return
                val ok = playerManager.selectVlcAudioTrack(pair.first)
                val selectedAfter = playerManager.getVlcSelectedAudioTrack()
                refreshCurrentAudioTrackUiState()
                android.util.Log.i("DDDPlayer/VLC", "audio_select targetId=${pair.first} ok=$ok selectedAfter=$selectedAfter displayedAudioLabel=${playerManager.getCurrentAudioTrackLabel()}")
        ...
        when (settingType) {
            SettingType.AUDIO_TRACK -> {
                if (playerManager.getActiveBackendId() == "VLC") {
                    val tracks = playerManager.getVlcAudioTracks()
                    val selectedBefore = playerManager.getVlcSelectedAudioTrack()
                    android.util.Log.i("DDDPlayer/VLC", "audio_cycle backend=${playerManager.getActiveBackendId()} tracks=${tracks.mapIndexed { idx, p -> "$idx:${p.first}:${p.second}" }} selectedBefore=$selectedBefore direction=$direction")
                    if (tracks.isNotEmpty()) {
                        val currentIndex = tracks.indexOfFirst { it.first == selectedBefore }.takeIf { it >= 0 } ?: 0
                        val targetIndex = (currentIndex + direction + tracks.size) % tracks.size
                        val target = tracks[targetIndex]
                        val ok = playerManager.selectVlcAudioTrack(target.first)
                        val selectedAfter = playerManager.getVlcSelectedAudioTrack()
                        refreshCurrentAudioTrackUiState()
                        android.util.Log.i("DDDPlayer/VLC", "audio_cycle targetIndex=$targetIndex targetId=${target.first} selectResult=$ok selectedAfter=$selectedAfter displayedAudioLabel=${playerManager.getCurrentAudioTrackLabel()}")
                        _toastMessage.postValue("VLC audio: ${target.second}")
                    }
                } else {
