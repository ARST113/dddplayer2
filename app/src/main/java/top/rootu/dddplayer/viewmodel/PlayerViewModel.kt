...
class PlayerViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun TrackOption.toFingerprint(trackType: Int): TrackFingerprint = TrackFingerprint(
        ...
        label = format?.label,
        normalizedName = normalizeTrackName(format?.label ?: nameFromMeta),
    ...
    private fun matchTrackByPreference(options: List<TrackOption>, preference: TrackFingerprint, mode: TrackRestoreMode = TrackRestoreMode.SMART): Int? {
        var bestIdx: Int? = null
        var bestScore = Int.MIN_VALUE
        options.forEachIndexed { i, option ->
            val fp = option.toFingerprint(preference.trackType)
            var score = 0
            if (!preference.formatId.isNullOrBlank() && preference.formatId == fp.formatId) score += 100
            if (!preference.language.isNullOrBlank() && preference.language == fp.language) score += 35
            if (!preference.normalizedName.isNullOrBlank() && preference.normalizedName == fp.normalizedName) score += 35
        ...
        val onlyOrdinal = bestScore == 10
        return when (mode) {
            TrackRestoreMode.SAFE -> if (onlyOrdinal) null else bestIdx
            TrackRestoreMode.SMART -> if (onlyOrdinal && options.size != audioOptions.size) null else bestIdx
            TrackRestoreMode.AGGRESSIVE -> bestIdx
        }
    }
    ...
    private fun emitTrackSelectionChanged(option: TrackOption, index: Int, trackType: String, reason: String, matchScore: Int?) {
        ...
        bridgeDispatcher?.emit(
            BridgeEvent.TrackSelectionChanged(
                ...
                label = option.format?.label ?: option.nameFromMeta,
