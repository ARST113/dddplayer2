...
class VlcBackend(
    ...
    fun getAudioTracks(): List<BackendAudioTrack> {
        ...
        val raw = list.filter { it.id != -1 }.map { BackendAudioTrack(it.id, it.name ?: "Track ${it.id}") }
        val selected = getSelectedAudioTrack()
        return raw.map { it.copy(selected = it.id == selected) }
    }
    ...
    fun getSelectedAudioTrack(): Int? {
        ...
        val fallback = lastSelectedAudioTrackId
        return fallback?.takeIf { id -> tracks.any { it.id == id } }
    }
