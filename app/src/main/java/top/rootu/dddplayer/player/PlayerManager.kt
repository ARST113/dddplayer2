...
//                .setMimeType(mimeType)
    ...
    fun getVlcAudioTracks(): List<Pair<Int, String>> = vlcBackend?.getAudioTracks() ?: emptyList()
    fun getVlcSelectedAudioTrack(): Int? = vlcBackend?.getSelectedAudioTrack()
    fun selectVlcAudioTrack(id: Int): Boolean = vlcBackend?.selectAudioTrack(id) == true
    fun getCurrentAudioTrackLabel(): String? = when (activeBackend) {
        vlcBackend -> {
            val selected = getVlcSelectedAudioTrack() ?: return null
            getVlcAudioTracks().firstOrNull { it.first == selected }?.second
        }
